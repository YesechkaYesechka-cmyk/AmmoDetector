...
void train_model(const std::string& model_path) {
    ...
    net << layers::conv(IMAGE_SIZE, IMAGE_SIZE, 7, 1, 32, padding::same) << activation::relu()
        << layers::max_pool(IMAGE_SIZE, IMAGE_SIZE, 32, 2)  // 256x256
        << layers::conv(256, 256, 5, 32, 64, padding::same) << activation::relu()
        << layers::max_pool(256, 256, 64, 2)  // 128x128
        << layers::conv(128, 128, 3, 64, 128, padding::same) << activation::relu()
        << layers::max_pool(128, 128, 128, 2)  // 64x64
        << layers::conv(64, 64, 3, 128, 256, padding::same) << activation::relu()
        << layers::max_pool(64, 64, 256, 2)  // 32x32
        << layers::conv(32, 32, 3, 256, 512, padding::same) << activation::relu()
        << layers::max_pool(32, 32, 512, 2)  // 16x16
        << fully_connected_layer(16 * 16 * 512, 1024) << activation::relu()
        << layers::dropout(1024, 0.5f)
        << fully_connected_layer(1024, static_cast<int>(num_classes)) << activation::softmax();
