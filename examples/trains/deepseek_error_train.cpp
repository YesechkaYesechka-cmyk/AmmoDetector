...
void train_model(const std::string& model_path) {
    ...
    // Упрощенная и более надежная архитектура
    network<sequential> net;

    /*// Первый сверточный блок
    net << convolutional_layer(IMAGE_SIZE, IMAGE_SIZE, 5, 1, 16)
        << activation::relu()
        << max_pooling_layer(IMAGE_SIZE, IMAGE_SIZE, 16, 2);  // 64x64x16

    // Второй сверточный блок
    net << convolutional_layer(64, 64, 3, 16, 32)
        << activation::relu()
        << max_pooling_layer(64, 64, 32, 2);  // 32x32x32

    // Третий сверточный блок
    net << convolutional_layer(32, 32, 3, 32, 64)
        << activation::relu()
        << max_pooling_layer(32, 32, 64, 2);  // 16x16x64

    // Рассчитываем правильный размер для полносвязного слоя
    const int fc_input_size = 16 * 16 * 64;  // 16384

    // Полносвязные слои
    net << fully_connected_layer(fc_input_size, 256)
        << activation::relu()
        << dropout_layer(256, 0.5f)
        << fully_connected_layer(256, static_cast<size_t>(num_classes))
        << activation::softmax();
    */

    // Точный расчет размеров БЕЗ padding:
    net << convolutional_layer(128, 128, 5, 1, 16)  // 124×124×16 (128-5+1=124)
        << activation::relu()
        << max_pooling_layer(124, 124, 16, 2);  // 62×62×16 (124/2=62)

    net << convolutional_layer(62, 62, 3, 16, 32)  // 60×60×32 (62-3+1=60)
        << activation::relu()
        << max_pooling_layer(60, 60, 32, 2);  // 30×30×32 (60/2=30)

    net << convolutional_layer(30, 30, 3, 32, 64)  // 28×28×64 (30-3+1=28)
        << activation::relu()
        << max_pooling_layer(28, 28, 64, 2);  // 14×14×64 (28/2=14)

        // РАСЧЕТ ДОЛЖЕН ИЗМЕНИТЬСЯ!
        const int fc_input_size = 14 * 14 * 64;  // 12544 (было 16384)

    net << fully_connected_layer(fc_input_size, 256)  // 12544 вместо 16384
        << activation::relu()
        << dropout_layer(256, 0.5f)
        << fully_connected_layer(256, static_cast<size_t>(num_classes))
        << activation::softmax();
    ...
    // // Проверяем размеры с помощью тестового примера
    // if (!images.empty()) {
    //     std::cout << "Testing network dimensions..." << std::endl;
    //     print_layer_sizes(net, images[0]);
    // }

    // Конфигурация обучения
    int batch_size = 4;
    int epochs = 20;  // Уменьшим для теста
    ...
    try {
        ...
        // Попробуем упрощенную архитектуру
        std::cout << "Trying simplified architecture..." << std::endl;

        network<sequential> simple_net;
        simple_net << fully_connected_layer(IMAGE_SIZE * IMAGE_SIZE, 64)
                   << activation::relu()
                   << fully_connected_layer(64, static_cast<size_t>(num_classes))
                   << activation::softmax();

        simple_net.fit<cross_entropy>(optimizer, images, labels, batch_size, epochs);
        simple_net.save(model_path + ".simple");
        std::cout << "Simple model saved to: " << model_path + ".simple" << std::endl;
    }
}
